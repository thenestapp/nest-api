...
export function isToolCallRequest(
    message?: Message,
): message is ParsedChatCompletionMessage<any> {
    ...
    if (state.children.length > 0) {
        return {
            ...
            children: state.children.map((child) =>
                addToolResponse(child, toolCallId, content),
            ),
        };
    }
    return state;
...
export const resumeCompletedToolCalls = (
    state: WorkflowState,
): WorkflowState => {
    const toolRequestMessage = state.messages.findLast(isToolCallRequest);
    if (toolRequestMessage) {
        const hasAllToolCalls = toolRequestMessage.tool_calls.every(
            (tollCall) =>
                state.messages.some(
                    (request) =>
                        "tool_call_id" in request &&
                        tollCall.id === request.tool_call_id,
                ),
        );
        if (hasAllToolCalls) {
            return {
...
export const getAllMissingToolCalls = (
    state: WorkflowState,
): ParsedFunctionToolCall[] => {
    ...
    if (state.children.length > 0) {
        missingToolCalls.push(
            ...state.children.flatMap(getAllMissingToolCalls),
        );
    }
