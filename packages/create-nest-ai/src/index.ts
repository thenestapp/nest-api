  ...
  options: [
    {
      ...
      label:
        "Surprise Trip Planner - travel agent creating personalized city adventures",
    },
    {
      value: {
        files: [
      ...
      label:
        "Medical Survey - Pre-visit patient questionnaire with report generation",
    },
    {
      value: {
        files: [
      ...
      label:
        "Library Photo to Website - Turn book shelf photos into browsable web catalogs",
    },
    {
      value: {
        files: [
...
if (!process.env.OPENAI_API_KEY) {
  ...
  if (choice === "later") {
    ...
    const apiKey = await text({
      ...
      validate: (value) =>
        value.length > 0 ? undefined : `Please provide a valid API key.`,
    });
