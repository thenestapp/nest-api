...
export async function downloadAndExtractTemplate(
  ...
  if (!response.ok || !response.body) {
    throw new Error(
      `Failed to fetch the code for example from ${response.url}.`,
    );
  }
  ...
  for (const file of filesToCopy) {
    ...
    execSync(
      `mv ${path.join(tmpdir(), `example/${file}`)} ${path.join(root, file)}`,
    );
  }
}
...
export async function copyAdditionalTemplateFiles(root: string) {
  ...
  for (const file of files) {
    execSync(
      `cp ${path.join(import.meta.dirname, file.src)} ${path.join(root, file.dest)}`,
    );
  }
}
