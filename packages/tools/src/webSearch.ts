...
export const createWebSearchTools = (options: SerplyOptions) => {
    ...
    return {
        googleSearch: tool({
            description:
                "Perform Google search with a search query using Serply API",
            parameters: z.object({
                query: z
                    .string()
                    .describe("Search query to use for Google search"),
            }),
            execute: async ({ query }) => {
                ...
                const parsedResponse = GoogleSearchResponseSchema.parse(
                    response.data,
                );
        ...
        googleImageSearch: tool({
            description:
                "Perform Google Image search with a search query using Serply API",
            parameters: z.object({
                query: z
                    .string()
                    .describe("Search query to use for Google Image search"),
            }),
            execute: async ({ query }) => {
                const response = await axios.get(
                    `https://api.serply.io/v1/search/q=${encodeURIComponent(query)}`,
                ...
                const parsedResponse = GoogleImageSearchResponseSchema.parse(
                    response.data,
                );
